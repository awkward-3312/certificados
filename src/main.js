...
const dashboardApp = {
  setup() {
    ...
    const paises = computed(() => [...new Set(certList.value.map(r => r.pais).filter(Boolean))]);
    const productos = computed(() => [...new Set(certList.value.map(r => r.tipo_producto).filter(Boolean))]);
    const certificadosTipo = computed(() => [...new Set(certList.value.map(r => r.tipo_certificado).filter(Boolean))]);

    const filtrados = computed(() => {
      return certList.value.filter(row => {
